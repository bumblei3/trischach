...
self.addEventListener('fetch', (event) => {
  ...
  const isStaticAsset = STATIC_ASSETS.some(asset => url.pathname === asset || url.pathname.endsWith(asset.replace('/', '')));
  
  if (isStaticAsset || request.destination === 'style' || request.destination === 'script' || request.destination === 'font') {
    // Cache first for static assets
    event.respondWith(CACHE_STRATEGIES.static(request, caches.open(STATIC_CACHE)));
  } else {
    // Network first for dynamic content
    event.respondWith(CACHE_STRATEGIES.networkFirst(request, caches.open(DYNAMIC_CACHE)));
  }
});
