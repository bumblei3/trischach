...
console.log(`║  Depth: ${AI_DEPTH} | Games per pairing: ${GAMES_PER_PAIRING}`.padEnd(61) + '║');
console.log('╚══════════════════════════════════════════════════════════════╝\n');
...
async function playGame(engineA, engineB, gameNum, totalGames) {
    ...
    if (currentEngine === 'random') {
      const actions = game.pieces
        ...
        .flatMap(p => {
          ...
          return [...moves.map(t => ({ piece: p, target: t, type: 'move' })),
  ...
  if (lastResult && lastResult.winner_faction) {
    ...
    const aMaterial = pieces.filter(p => factionToEngine[p.faction] === engineA).length;
    const bMaterial = pieces.filter(p => factionToEngine[p.faction] === engineB).length;
    const balancedMaterial = pieces.filter(p => factionToEngine[p.faction] === 'balanced').length;
  ...
  return { scoreA, moves: moveCount, winner: lastResult?.winner_faction || null };
}
...
async function runTournament() {
  ...
  for (const [engineA, engineB] of pairings) {
    ...
    console.log(`   ${engineA}: ${aStats.wins}W ${aStats.draws}D ${aStats.losses}L | Elo: ${eloRatings[engineA]}`);
    console.log(`   ${engineB}: ${bStats.wins}W ${bStats.draws}D ${bStats.losses}L | Elo: ${eloRatings[engineB]}`);
  }
  ...
  sorted.forEach((engine, idx) => {
    let totalW = 0, totalD = 0, totalL = 0, totalG = 0;
    engines.forEach(op => {
      if (results[engine][op]) {
        totalW += results[engine][op].wins;
        totalD += results[engine][op].draws;
        totalL += results[engine][op].losses;
        totalG += results[engine][op].wins + results[engine][op].draws + results[engine][op].losses;
      }
    });
    const delta = eloRatings[engine] - INITIAL_ELO;
    const deltaStr = delta >= 0 ? `+${delta}` : `${delta}`;
    console.log(`${String(idx + 1).padStart(4)} | ${engine.padEnd(15)} | ${String(eloRatings[engine]).padStart(4)} | ${deltaStr.padStart(4)} | ${String(totalW).padStart(3)} | ${String(totalD).padStart(3)} | ${String(totalL).padStart(3)} | ${String(totalG).padStart(5)}`);
  });

  // Cross table
  console.log('\n📋 CROSS TABLE (Row vs Column - % score):');
  console.log('Engine'.padEnd(15), engines.map(e => e.substring(0, 4)).join('  '));
