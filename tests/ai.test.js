...
describe('AI Decision Making (Minimax)', () => {
  ...
  test('prioritizes advantageous attack over moving', () => {
    ...
    const naturePawn = new Piece(PIECE_TYPE.PAWN, FACTION.NATURE, new Hex(0, 0));
  ...
  test('moves towards the center heuristic', () => {
    ...
    const distFromCenter = Math.max(
      Math.abs(fireKnight.pos.q), Math.abs(fireKnight.pos.r),
  ...
  test('minimax: prefers capturing high-value piece over low-value', () => {
    ...
    const natureQueen = new Piece(PIECE_TYPE.QUEEN, FACTION.NATURE, new Hex(-1, 0));
  ...
  test('minimax: game state unchanged after AI calculation', () => {
    ...
    const naturePawn = new Piece(PIECE_TYPE.PAWN, FACTION.NATURE, new Hex(1, 1));
    ...
    const stateBefore = {
      pieces: game.getAlivePieces().map(p => ({
        type: p.type, faction: p.faction, q: p.pos.q, r: p.pos.r, alive: p.alive
      })),
