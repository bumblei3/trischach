...
describe('RPS Attack Categorization', () => {
  ...
  test('select returns rpsAttacks when RPS is enabled', () => {
    ...
    game.pieces = [fireQueen, naturePawn, waterPawn, fireKing, natureKing, waterKing];
    ...
    expect(result.rpsAttacks.advantage.some(h => h.equals(new Hex(1, 0)))).toBe(true);
    // Water is disadvantage for Fire
    expect(result.rpsAttacks.disadvantage.some(h => h.equals(new Hex(-1, 1)))).toBe(true);
  });
  ...
  test('rpsAttacks is null when no attacks possible', () => {
    ...
    const natureKing = new Piece(PIECE_TYPE.KING, FACTION.NATURE, new Hex(5, -5));
    const waterKing = new Piece(PIECE_TYPE.KING, FACTION.WATER, new Hex(-5, 5));
    game.pieces = [fireQueen, f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, fireKing, natureKing, waterKing];
    game._rebuildOccupiedMap();
