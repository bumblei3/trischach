...
describe('AI Core: Dynamic Piece Values (RPS-aware)', () => {
  ...
  test('Material values change based on RPS relationship', () => {
    ...
    const firePawn = game.pieces.find(p => p.faction === FACTION.FIRE && p.type === 'pawn');
...
describe('Game Replay: TSPN Serialization', () => {
  ...
  test('TSPN captures RPS results in notation', () => {
    ...
    const firePawn = testGame.pieces.find(p => p.faction === FACTION.FIRE && p.type === 'pawn');
    const waterPawn = testGame.pieces.find(p => p.faction === FACTION.WATER && p.type === 'pawn');
...
describe('Auto-Battle Integration', () => {
  ...
  test('Auto-battle handles combat correctly', () => {
    ...
    const natureKing = new Piece(PIECE_TYPE.KING, FACTION.NATURE, new Hex(-2, 2));
  ...
  test('Game ends correctly when one faction remains', () => {
    ...
    game.eliminatedFactions.add(FACTION.NATURE);
    game.pieces.forEach(p => {
      if (p.faction === FACTION.WATER || p.faction === FACTION.NATURE) p.alive = false;
    });
    game._rebuildOccupiedMap();
    ...
    expect(game.state === GAME_STATE.GAME_OVER || game.currentFaction === FACTION.FIRE).toBe(true);
  });
});
