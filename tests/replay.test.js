...
function createMoveHistory() {
  return [
    {
      piece: { id: 'p1', type: 'pawn', faction: FACTION.FIRE, pos: new Hex(0, 5), symbol: 'P' },
      to: new Hex(0, 4),
      action: 'move',
      faction: FACTION.FIRE,
    },
    {
      piece: { id: 'p2', type: 'pawn', faction: FACTION.WATER, pos: new Hex(0, 0), symbol: 'P' },
      to: new Hex(0, 1),
      action: 'move',
      faction: FACTION.WATER,
    },
    {
      piece: { id: 'p3', type: 'pawn', faction: FACTION.NATURE, pos: new Hex(-2, 2), symbol: 'P' },
      to: new Hex(-1, 2),
      action: 'move',
...
describe('Replay: Serialization Helpers', () => {
  ...
  describe('wrapLine', () => {
    ...
    test('wraps at word boundaries', () => {
      const result = wrapLine('this is a very long line that should wrap at word boundaries', 20);
...
describe('Replay: formatMove', () => {
  ...
  test('formats capture with RPS advantage', () => {
    const move = {
      piece: { id: 'p1', type: 'queen', faction: FACTION.FIRE, pos: new Hex(0, 1), symbol: 'Q' },
      to: new Hex(0, 0),
      action: 'combat',
      faction: FACTION.FIRE,
      rpsResult: 'advantage',
  ...
  test('formats capture with RPS disadvantage', () => {
    const move = {
      piece: { id: 'p1', type: 'pawn', faction: FACTION.FIRE, pos: new Hex(0, 1), symbol: 'P' },
      to: new Hex(0, 0),
      action: 'combat',
      faction: FACTION.FIRE,
  ...
  test('handles promotion-only entry (no target)', () => {
    const move = {
      action: 'promotion',
      piece: { id: 'p1', type: 'pawn', faction: FACTION.FIRE, pos: new Hex(0, 0), symbol: 'P' },
    };
  ...
  test('handles unknown faction fallback', () => {
    const move = {
      piece: { id: 'p1', type: 'pawn', faction: 'unknown', pos: new Hex(0, 5), symbol: 'P' },
      to: new Hex(0, 4),
      action: 'move',
      faction: 'unknown',
    };
...
describe('Replay: serializeGame', () => {
  ...
  test('wraps long lines at 80 chars', () => {
    ...
    const moves = Array.from({ length: 20 }, (_, i) => ({
      piece: { id: `p${i}`, type: 'pawn', faction: FACTION.FIRE, pos: new Hex(i % 7 - 3, 5), symbol: 'P' },
      to: new Hex(i % 7 - 3, 4),
      action: 'move',
      faction: FACTION.FIRE,
    }));
...
describe('Replay: ReplayController', () => {
  ...
  beforeEach(() => {
    mockGame = createMockGame({
      handleCellClick: vi.fn((pos) => pos ? { promotion: false } : { promotion: false }),
    });
    moveHistory = createMoveHistory();
  });
...
describe('Replay: reconstructGameFromTSPN', () => {
  test('creates game and controller from parsed TSPN', () => {
    ...
    const mockPieces = [
      { id: 'p1', type: 'pawn', faction: 'fire', pos: { q: 0, r: 5 }, symbol: 'P', alive: true, hasMoved: false },
    ];
    ...
    const { game, controller } = reconstructGameFromTSPN(parsedTSPN, MockGameClass, boardCells);
  ...
  test('disables RPS when header is off', () => {
    ...
    const { game } = reconstructGameFromTSPN(parsedTSPN, MockGameClass, boardCells);
