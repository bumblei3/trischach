...
class MockGame {
  ...
  _setupStartingPosition() {
    ...
    let fireIdx = 0, waterIdx = 0, natureIdx = 0;
...
describe('Opening Book: buildOpeningBook', () => {
  ...
  test('creates entries for all 12 opening lines', () => {
    ...
    const totalVariations = Array.from(OPENING_BOOK.values()).reduce((sum, arr) => sum + arr.length, 0);
  ...
  test('returns valid piece for the move', () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (move) {
        ...
        const matchingEntry = bookMoves.find(m => m.move.pieceId === move.piece.id);
