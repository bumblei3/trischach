...
export function generateBoard(): Map<string, Cell> {
  ...
  for (let d = 1; d <= 2; d++) {
    ...
    for (let r = -d; r <= N; r++) {
      const hex = new Hex(q, r);
      cells.set(hex.key, { hex, zone: ZONE.START_WATER, faction: FACTION.WATER });
    }
  }
  ...
  for (let d = 1; d <= 2; d++) {
    ...
    for (let r = -d; r <= N; r++) {
      const q = -r - s;
      const hex = new Hex(q, r);
      cells.set(hex.key, { hex, zone: ZONE.START_NATURE, faction: FACTION.NATURE });
    }
  }
...
export class BoardRenderer {
  ...
  public render(): void {
    ...
    for (const [key, cell] of this.cells) {
      ...
      label.setAttribute('y', String(px.y + this.hexSize * 0.70));
      ...
      g.appendChild(cg);
      this.hexElements.set(key, { group: cg, polygon: poly, label: label, cell });
    }
  ...
  public removePiece(id: string): void {
    ...
    if (e) { e.element.remove(); this.pieceElements.delete(id); }
  }
  ...
  public async animateMove(piece: { id: string; pos: Hex }, _from: Hex, to: Hex): Promise<void> {
