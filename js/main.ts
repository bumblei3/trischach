...
import { BoardRenderer, FACTION_COLORS, FACTION, generateBoard } from './board.ts';
...
const depthNames: Record<number, string> = { 1: 'Leicht', 2: 'Mittel', 3: 'Schwer', 4: 'Extrem' };
...
const promotionOverlay = document.getElementById('promotion-overlay') as HTMLElement;
...
function applySettings(settings: GameSettings): void {
  ...
  if (depthLabel) depthLabel.textContent = 'KI: ' + depthNames[settings.aiDepth];
...
function addToLog(result: { piece: { faction: string; symbol: string }; notation: string }): void {
  const moveLogEl = document.getElementById('move-log') as HTMLElement;
...
let contextMenuPiece: { id: string; type: string; faction: string; pos: { q: number; r: number }; symbol: string } | null = null;

renderer.onPieceLongPress = (piece: { id: string; type: string; faction: string; pos: { q: number; r: number }; symbol: string }, position: { clientX: number; clientY: number }) => {
...
function showContextMenu(piece: { id: string; type: string; faction: string; pos: { q: number; r: number }; symbol: string }, position: { clientX: number; clientY: number }): void {
...
function handleContextMenuAction(action: string | undefined, piece: { id: string; type: string; faction: string; pos: { q: number; r: number }; symbol: string }): void {
  if (!action) return;
...
function showPromotion(piece: { id: string; type: string; faction: string; pos: { q: number; r: number }; symbol: string }): void {
  ...
  promotionOverlay.querySelectorAll('.promotion-choice').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (!newType) return;
      promotionOverlay.classList.remove('visible');
      const result = game.completePromotion(newType as 'queen' | 'rook' | 'bishop' | 'knight');
      if (result) {
        handlePromotionResult(result, piece);
      }
    });
...
function initEventListeners(): void {
  ...
  rpsToggle?.addEventListener('change', (e: Event) => {
    ...
    updateUI();
    saveSettings({ ...loadSettings(), rpsEnabled: (e.target as HTMLInputElement).checked });
  });

  const soundToggle = document.getElementById('sound-toggle') as HTMLInputElement;
  soundToggle?.addEventListener('change', (e: Event) => {
    sounds.toggle((e.target as HTMLInputElement).checked);
    saveSettings({ ...loadSettings(), soundEnabled: (e.target as HTMLInputElement).checked });
  });
  ...
  function updateReplayUI(): void {
    ...
    const replayFirst = document.getElementById('replay-first') as HTMLButtonElement;
    const replayPrev = document.getElementById('replay-prev') as HTMLButtonElement;
    const replayNext = document.getElementById('replay-next') as HTMLButtonElement;
    const replayLast = document.getElementById('replay-last') as HTMLButtonElement;
    const replayPlay = document.getElementById('replay-play') as HTMLButtonElement;
    const replayPause = document.getElementById('replay-pause') as HTMLButtonElement;
  ...
  function replayPlay(): void {
    ...
    const speed = parseFloat((document.getElementById('replay-speed') as HTMLInputElement)?.value || '1');
  ...
  function applyGameState(state: any): void {
    ...
    game.capturedPieces = {
      fire: (state.capturedPieces.fire || []).map((id: string) => game.pieces.find((p: any) => p.id === id)).filter(Boolean),
      water: (state.capturedPieces.water || []).map((id: string) => game.pieces.find((p: any) => p.id === id)).filter(Boolean),
      nature: (state.capturedPieces.nature || []).map((id: string) => game.pieces.find((p: any) => p.id === id)).filter(Boolean),
    };
  ...
  const replayPauseBtn = document.getElementById('replay-pause') as HTMLButtonElement;
  const replayNext = document.getElementById('replay-next') as HTMLButtonElement;
  const replayLast = document.getElementById('replay-last') as HTMLButtonElement;
