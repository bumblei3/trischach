...
function buildCompiledBook(source) {
    ...
    for (const [, lineData] of Object.entries(factionLines)) {
      ...
      for (let i = 0; i < moves.length; i++) {
        ...
        if (!selectResult || selectResult.action !== 'select') {
          stats.warnings.push(`${name}: Failed to select piece ${parsed.piece.id} at ply ${i}`);
          break;
        }
  ...
  stats.totalVariations = Array.from(compiledBook.values()).reduce((sum, arr) => sum + arr.length, 0);
...
async function loadCompiledBook() {
  try {
    ...
    console.log(`Loaded compiled book: ${data.metadata.stats.totalPositions} positions`);
    return data;
  } catch {
