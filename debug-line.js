...
for (let i = 0; i < moves.length; i++) {
  ...
  if (!piece) {
    ...
    const alive = game.pieces.filter(p => p.faction === game.currentFaction && p.alive);
    console.log(`  Alive ${game.currentFaction} pieces:`, alive.map(p => p.id));
    break;
  }
  ...
  // Check valid moves
  const validMoves = game.getLegalMoves ? game.getLegalMoves(piece) : { moves: [], attacks: [] };
  const allTargets = [...validMoves.moves, ...validMoves.attacks].map(h => h.key);
  ...
  const result = game.handleCellClick(new (await import("./js/hex.js")).Hex(q, r));
